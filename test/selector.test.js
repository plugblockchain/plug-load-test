...
describe('Send_and_receive_selection', function() {
  it('Random_testing', function() {
    const keypairs = ['A', 'B', 'C', 'D', 'E'];
    let sender_last = -1;
    const sel = new selector.KeypairSelector(keypairs.slice(0));

    // probability of randomness resulting in missing a failure = (1 - (4/5)^25) = 0.3%
    for (i=0; i<100; i++) {
      const [sender, receiver] = sel.next();

      assert.ok( keypairs.includes(sender));
      assert.ok( keypairs.includes(receiver));
      assert.notEqual( sender, receiver);
      assert.notEqual( sender_last, sender );
      sender_last = sender;
    }
  });
