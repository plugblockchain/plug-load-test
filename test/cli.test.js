...
describe('Command Line Interface - API Selection', function() {
  it('Default api', function() {
    process.argv = ['node','cli.test.js'];
    result = cli.parseCliArguments();
    expected = 'cennznet';
    assert.equal(result.api, expected);
  });
  it('Set cennznet', function() {
    process.argv = ['node','cli.test.js', '--cennznet'];
    result = cli.parseCliArguments();
    expected = 'cennznet';
    assert.equal(result.api, expected);
  });
  it('Set plug', function() {
    process.argv = ['node','cli.test.js', '--plug'];
    result = cli.parseCliArguments();
    expected = 'plug';
    assert.equal(result.api, expected);
  });
  it('Set cennznet and plug', function() {
    process.argv = ['node','cli.test.js', '--plug', '--cennznet'];
    result = cli.parseCliArguments();
    expected = 'cennznet';
    assert.equal(result.api, expected);
  });
